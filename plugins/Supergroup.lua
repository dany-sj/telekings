...
function get_message_callback(extra, success, result)
	...
	elseif get_cmd == "حذف ادمین" then
		...
		send_large_msg(channel_id, text)
	elseif get_cmd == "تنظیم صاحب گروه" then
...
local function run(msg, matches)
	...
	local data = load_data(_config.moderation.data)
		...
		if matches[1] == "لیست" and not matches[2] and is_momod(msg) then
		...
		if matches[1] == 'ازاد کردن قفل' and is_momod(msg) then
			local target = msg.to.id
			     if matches[2] == 'all' then
      	local dsafemode ={
        unlock_group_links(msg, data, target),
		...
		if matches[1] == "silent" or matches[1] == "unsilent" and is_momod(msg) then
			...
			elseif matches[1] == "خفه" or matches[1] == "ازاد کردن خفه" and not string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local get_cmd = "mute_user"
				local username = matches[2]
				local username = string.gsub(matches[2], '@', '')
